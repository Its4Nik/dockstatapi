...
RUN apk add --update --no-cache bash curl nodejs && \
    adduser -h /api -s /bin/bash -D dockstatapi dockstatapi && \
    chown -hR dockstatapi:dockstatapi /api
...
COPY --chown=dockstatapi:dockstatapi --from=main /build /api
